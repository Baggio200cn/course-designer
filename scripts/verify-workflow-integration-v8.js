...
test('CodexR2 · script 变化时停止朗读（防旧闭包进度/内容不一致）', () => {
  ...
  assert(/useEffect\([\s\S]{0,400}cancelledRef\.current = true[\s\S]{0,400}\[script\]\)/.test(src),
    'script 变化未停止当前朗读');
});
...
// ── 【32】功能5+ 周老师真声（声音复刻）混合模式 ──────────────────────────────
console.log('\n【32】功能5+ 周老师真声（声音复刻）· 凭据走配置 + IPC + 选段试听');

test('VoiceClone · 客户端 selfCheck 通过（buildSynthesisBody/synthesizeVoice 导出）', () => {
  const mod = require(path.resolve(__dirname, '..', 'src', 'main', 'api', 'volc-voice-clone.js'));
  assert(typeof mod.synthesizeVoice === 'function', '缺 synthesizeVoice 导出');
  assert(typeof mod.buildSynthesisBody === 'function', '缺 buildSynthesisBody 导出');
  assert(typeof mod.selfCheck === 'function', '缺 selfCheck 导出');
  const r = mod.selfCheck();
  assert(r && r.passed === r.total && r.total > 0, `selfCheck 未全过：${r && r.passed}/${r && r.total}`);
});

test('VoiceClone · 凭据走配置不硬编码（源码无明文 x-api-key / speakerId）', () => {
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'src', 'main', 'api', 'volc-voice-clone.js'), 'utf8');
  // 不允许把真实凭据写死在源码（重蹈 Ark key 覆辙）
  assert(!/1c85da94-96d8-43e2-aedf-123bb9ad0ff9/.test(src), '源码硬编码了真实 x-api-key');
  assert(!/S_zxFSBjj42/.test(src), '源码硬编码了真实 speakerId');
});

test('VoiceClone · IPC handler v2:synthesizeLectureVoice 从配置读凭据', () => {
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'src', 'main', 'ipc', 'system.handlers.js'), 'utf8');
  assert(/v2:synthesizeLectureVoice/.test(src), '缺 v2:synthesizeLectureVoice handler');
  assert(/voice_clone_api_key/.test(src) && /voice_clone_speaker_id/.test(src),
    'handler 未从配置读 voice_clone_api_key / voice_clone_speaker_id');
});

test('VoiceClone · preload 暴露 synthesizeLectureVoiceV2', () => {
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'src', 'preload', 'index.js'), 'utf8');
  assert(/synthesizeLectureVoiceV2/.test(src), 'preload 未暴露 synthesizeLectureVoiceV2');
});

test('VoiceClone · API 配置面板含声音复刻两个字段', () => {
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'src', 'renderer', 'src', 'v2', 'V2App.jsx'), 'utf8');
  assert(/voice_clone_api_key/.test(src) && /voice_clone_speaker_id/.test(src),
    'V2App 未保存/读取声音复刻凭据');
});

test('VoiceClone · LectureReader 选段试听走真声合成（混合模式）', () => {
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'src', 'renderer', 'src', 'v2', 'LectureReader.jsx'), 'utf8');
...
// ── 【36】codex 复审收口（2026-05-30）：save 刷新/下游 dirty + 异步竞态 + 下拉一致 + DB 不变量 ──
console.log('\n【36】codex 复审收口 · save 联动刷新/dirty + loadSeq 竞态防护 + 下拉一致 + DB 撤销不变量');

test('codex复审#1 · quiz/homework 保存 markDownstreamDirty + autoSave 通知父级刷新', () => {
  ...
  const quizU = fs.readFileSync(path.resolve(ui, 'QuizStage.jsx'), 'utf8');
  const hwU = fs.readFileSync(path.resolve(ui, 'HomeworkStage.jsx'), 'utf8');
  assert(/markDownstreamDirty\(notebookId, 'quiz', 'quiz-edited'\)/.test(quizH), 'quizSave 未标下游 dirty');
  assert(/markDownstreamDirty\(notebookId, 'homework', 'homework-edited'\)/.test(hwH), 'homeworkSave 未标下游 dirty');
  // autoSave 成功后调 onStageDataChanged（不止 confirm）
  assert(/已自动保存（quiz[\s\S]{0,300}onStageDataChanged\?\.\(\)/.test(quizU), 'QuizStage autoSave 未通知父级刷新');
  assert(/已自动保存（homework[\s\S]{0,300}onStageDataChanged\?\.\(\)/.test(hwU), 'HomeworkStage autoSave 未通知父级刷新');
});

test('codex复审#2/#3 · loadSeq 竞态防护 + 下拉文案用优先 confirmed', () => {
  const ui = path.resolve(__dirname, '..', 'src', 'renderer', 'src', 'v2');
  ...
  for (const [name, src] of [['QuizStage', quiz], ['HomeworkStage', hw]]) {
    assert(/loadSeqRef = useRef\(0\)/.test(src), `${name} 缺 loadSeqRef 竞态防护`);
    assert(/const seq = \+\+loadSeqRef\.current/.test(src) && /if \(seq !== loadSeqRef\.current\) return/.test(src),
      `${name} effect 未用 seq 丢弃迟到响应`);
  }
  assert(/const hasQuiz = pickPreferredQuizByLesson\(savedQuizzes, l\.lessonNumber\)/.test(quiz),
    'QuizStage 下拉文案未用优先 confirmed');
  assert(/const hasHw = pickPreferredHomeworkByLesson\(savedHomeworks, l\.lessonNumber\)/.test(hw),
    'HomeworkStage 下拉文案未用优先 confirmed');
});

test('codex复审#4 行为 · 撤销确认即使显式传 lifecycle 也强制清 confirmedAt（DB 不变量封死）', () => {
  const os = require('os');
  const tmp = path.join(os.tmpdir(), 'yuke-db-test2-' + Date.now() + '-' + Math.floor(Math.random() * 1e6));
  fs.mkdirSync(tmp, { recursive: true });
  process.env.ELECTRON_USER_DATA = tmp;
  const dbModPath = path.resolve(__dirname, '..', 'src', 'main', 'database', 'db-simple.js');
  delete require.cache[require.resolve(dbModPath)];
  ...
  const db = new DatabaseManager();
  try {
    const a = db.createArtifact({ notebookId: 999002, type: 'quiz_set', stage: 'quiz', content: {}, status: 'draft', confirmed: false, metadata: {} });
    db.updateArtifact(a.id, { confirmed: true, status: 'confirmed', confirmedAt: new Date().toISOString() });
    // 撤销确认但显式带了一个 lifecycle（不含 confirmedAt:null）→ 仍必须清空
    const saved = db.updateArtifact(a.id, { status: 'draft', confirmed: false, lifecycle: { note: 'x' } });
    assert(!(saved.lifecycle && saved.lifecycle.confirmedAt),
      '显式传 lifecycle 时撤销确认未强制清 lifecycle.confirmedAt（不变量未封死）');
    assert(saved.lifecycle && saved.lifecycle.note === 'x', '显式 lifecycle 其它字段应保留');
  } finally {
    try { fs.rmSync(tmp, { recursive: true, force: true }); } catch (_) {}
  }
});
...
test('codex3轮#2/#3 · 详情加载 loadSeq 守卫 + 重新生成强制新建（autoSave idArg）', () => {
  ...
  const quiz = fs.readFileSync(path.resolve(ui, 'QuizStage.jsx'), 'utf8');
  const hw = fs.readFileSync(path.resolve(ui, 'HomeworkStage.jsx'), 'utf8');
  // #2 详情请求纳入 seq 守卫（第4轮起 seq 自增，故 ++ 也算）
...
// ── 【38】codex 第4轮复审（2026-05-30）：同笔记本切课竞态 + 真 IPC handler 行为测试 ──
console.log('\n【38】codex 第4轮复审 · 详情 seq 自增（同笔记本切课）+ v2:quizSave 真 handler 行为');

test('codex4轮#1 · loadQuiz/loadHomework 详情 seq 自增（同笔记本切课也防乱序覆盖）', () => {
  const ui = path.resolve(__dirname, '..', 'src', 'renderer', 'src', 'v2');
  const quiz = fs.readFileSync(path.resolve(ui, 'QuizStage.jsx'), 'utf8');
  const hw = fs.readFileSync(path.resolve(ui, 'HomeworkStage.jsx'), 'utf8');
  assert(/const seq = \+\+loadSeqRef\.current;[\s\S]{0,120}quizGetV2/.test(quiz),
    'loadQuiz 详情 seq 未自增（同笔记本切课仍会乱序覆盖）');
  assert(/const seq = \+\+loadSeqRef\.current;[\s\S]{0,120}homeworkGetV2/.test(hw),
    'loadHomework 详情 seq 未自增');
});

test('codex4轮#2 行为 · v2:quizSave 撤销确认后持久化 workflow 真的移除 report', () => {
  const os = require('os');
  const tmp = path.join(os.tmpdir(), 'yuke-db-test4-' + Date.now() + '-' + Math.floor(Math.random() * 1e6));
  fs.mkdirSync(tmp, { recursive: true });
  process.env.ELECTRON_USER_DATA = tmp;
  const dbModPath = path.resolve(__dirname, '..', 'src', 'main', 'database', 'db-simple.js');
  delete require.cache[require.resolve(dbModPath)];
  const DatabaseManager = require(dbModPath);
  const { computeUnlockedStages } = require(path.resolve(__dirname, '..', 'src', 'main', 'v2', 'contracts.js'));
  const { register } = require(path.resolve(__dirname, '..', 'src', 'main', 'ipc', 'v2', 'quiz.handlers.js'));
  const db = new DatabaseManager();
  try {
    const NB = 999004;
    const reqs = [['schedule', 'schedule_table'], ['design', 'design_doc'], ['ppt', 'ppt_outline'],
    ...
    let quizArtId = null;
    for (const [stage, type] of reqs) {
      const a = db.createArtifact({ notebookId: NB, type, stage, content: { questions: [] }, status: 'confirmed', confirmed: true, metadata: { lessonNumber: 1 } });
      if (stage === 'quiz') quizArtId = a.id;
    }
    // 持久化 workflow（含 report 已解锁）——模拟之前已解锁的状态
    ...
    // 注入真 db + 一个走 computeUnlockedStages 重算的 sync，注册 handler 并真实调用
    const syncWFA = (nid) => {
      const arts = db.listArtifacts({ notebookId: nid });
      const unlockedStages = computeUnlockedStages(arts);
      return db.upsertWorkflowState(nid, { ...(db.getWorkflowState(nid) || {}), unlockedStages });
    };
    const handlers = {};
    const ipcMain = { handle: (ch, fn) => { handlers[ch] = fn; } };
    register(ipcMain, () => ({ db, syncWorkflowStageAvailability: syncWFA }));
    assert(typeof handlers['v2:quizSave'] === 'function', '未注册 v2:quizSave handler');
    // 真实调用 handler 撤销确认（编辑保存）。quizSave 内部全同步（无内部 await），
    // 调用后 db 写入即时完成，故同步检查持久化 workflow 即可（test() 仅支持同步）。
    handlers['v2:quizSave'](null, { notebookId: NB, quizId: quizArtId, metadata: { lessonNumber: 1 }, content: { questions: [{ q: 'x' }] } });
    const after = db.getWorkflowState(NB).unlockedStages || [];
    assert(!after.includes('report'), 'quizSave 撤销确认后持久化 workflow 仍含 report（未触发重算）');
  } finally {
    try { fs.rmSync(tmp, { recursive: true, force: true }); } catch (_) {}
  }
});
...
// ── 【39】报告导出不丢手填内容 + 导出守卫（老师测试 2026-05-30）──────────────
console.log('\n【39】报告导出 · implementationOutcomes 渲染到导出物（不丢内容）+ 未保存导出守卫');

test('报告导出行为 · exportReportMarkdown 把 implementationOutcomes 渲染进导出（不显示"未填"）', () => {
  const os = require('os');
  const { exportReportMarkdown } = require(path.resolve(__dirname, '..', 'src', 'main', 'export', 'report-export.js'));
  const out = path.join(os.tmpdir(), 'yuke-report-md-' + Date.now() + '-' + Math.floor(Math.random() * 1e6) + '.md');
  const report = {
    courseName: '测试课', implementationOutcomes: {
      studentEngagement: { achieved: '达成内容XYZ唯一标记', evidence: '证据内容ABC唯一标记' },
    },
    reflectionAndImprovement: { achievements: ['成效一'] },
  };
  try {
    exportReportMarkdown({ report, outputPath: out });
    const md = fs.readFileSync(out, 'utf8');
    assert(md.includes('达成内容XYZ唯一标记'), '导出未渲染 implementationOutcomes.achieved（手填内容丢失）');
    assert(md.includes('证据内容ABC唯一标记'), '导出未渲染 implementationOutcomes.evidence（手填内容丢失）');
    // 已填的"学生参与度·达成情况"那一行不应是"（未填）"
    assert(/学生参与度\s*\n- 达成情况：达成内容XYZ唯一标记/.test(md), '已填的学生参与度被导出成"未填"');
  } finally {
    try { fs.rmSync(out, { force: true }); } catch (_) {}
  }
});

test('报告导出守卫 · 有未保存修改时先确认再导出（ReportStage onExport）', () => {
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'src', 'renderer', 'src', 'v2', 'ReportStage.jsx'), 'utf8');
  assert(/const onExport = async \(format\) => \{[\s\S]{0,400}if \(dirty\)[\s\S]{0,200}window\.confirm/.test(src),
    'onExport 未在 dirty 时做导出前确认守卫');
  assert(/if \(dirty\)[\s\S]{0,300}await onSave\(\)/.test(src),
    'onExport 守卫未在确认后先保存（仍可能导出旧版）');
});

test('报告底部保存按钮 · 无改动时显示"已保存"（不再灰着却写"保存修改"）', () => {
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'src', 'renderer', 'src', 'v2', 'ReportStage.jsx'), 'utf8');
  assert((src.match(/dirty \? '💾 保存修改' : '✓ 已保存'/g) || []).length >= 2,
    '底部保存按钮文案未随 dirty 切换（顶部+底部应一致）');
});

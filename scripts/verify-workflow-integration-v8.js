...
test('Round 15 P1.1 · report.handlers 保留 7 个上游 artifact 对象（不只 .content）', () => {
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'src', 'main', 'ipc', 'v2', 'report.handlers.js'), 'utf8');
  // v4.3.3 Round 16 重构后两种命名都允许：① upstream.objs.* 解构出来 ② 直接 const xxxArt
  // 验证 7 阶段 artifact 类型都被收集（pickLatestConfirmed 调用 7 次或在 helper 内部）
  ['schedule_table', 'design_doc', 'ppt_outline', 'lecture_final', 'quiz_set', 'homework_set', 'video_prompt'].forEach((t) => {
    assert(src.includes(t),
      `report.handlers 未引用 type=${t}（应在 collectReportUpstream / generateReport 中收集）`);
  });
});
...
test('Round 16 · report.handlers 抽出 collectReportUpstream / inferReportSourceArtifactIds helper', () => {
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'src', 'main', 'ipc', 'v2', 'report.handlers.js'), 'utf8');
  assert(/function collectReportUpstream\(/.test(src),
    'report.handlers 缺 collectReportUpstream helper（Codex Round 16）');
  assert(/function inferReportSourceArtifactIds\(/.test(src),
    'report.handlers 缺 inferReportSourceArtifactIds helper（Codex Round 16）');
});
...
test('Round 16 · saveReport 新建分支兜底顺序：先继承上份 report，再从 7 个上游重建', () => {
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'src', 'main', 'ipc', 'v2', 'report.handlers.js'), 'utf8');
  // inferReportSourceArtifactIds 必须先尝试继承 prevReport.sourceArtifactIds，再从 7 类 artifact 重建
...
test('Round 16 · 行为验证 · inferReportSourceArtifactIds 在 mock db 上能正确推断', () => {
  // 拉模块直接做行为验证（不通过 IPC）
  const modPath = path.resolve(__dirname, '..', 'src', 'main', 'ipc', 'v2', 'report.handlers.js');
  // handler 不直接 export，但我们可以通过 source-level 评估（避免引入 Electron deps）。
  // 这里做 lightweight 检查：函数体含必要逻辑分支。
  const src = fs.readFileSync(modPath, 'utf8');
  // case 1: 有上一份 report 且 sourceArtifactIds 非空 → 复用
  const fn = src.match(/function inferReportSourceArtifactIds[\s\S]*?\n\}/);
  assert(fn, '找不到 inferReportSourceArtifactIds 函数定义');
  const body = fn[0];
  // 必须先过滤 type==='implementation_report'
  assert(/type\s*===\s*['"]implementation_report['"]/.test(body),
    'inferReportSourceArtifactIds 未筛选 implementation_report');
  // case 2: 重建路径必须涵盖 7 种 type
  ['schedule_table', 'design_doc', 'ppt_outline', 'lecture_final', 'quiz_set', 'homework_set', 'video_prompt'].forEach((t) => {
    assert(body.includes(t),
      `inferReportSourceArtifactIds rebuild 分支缺 type=${t}`);
  });
});

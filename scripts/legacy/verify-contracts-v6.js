...
test('STAGE_ORDER 是 6 阶段（v4.2.0 调换 lecture/ppt）', () => {
  // 2026-05-16 v4.2.0 Phase A'：ppt 在 lecture 之前
  assertEqual(STAGE_ORDER, ['schedule', 'design', 'ppt', 'lecture', 'video', 'report'], 'STAGE_ORDER');
});
...
test('ppt 依赖 design_doc（v4.2.0 改为基于设计直接生成 PPT）', () => {
  assertEqual(STAGE_REQUIREMENTS.ppt, [{ type: 'design_doc', stage: 'design' }], 'ppt 依赖');
});

test('lecture 依赖 design_doc + ppt_outline（v4.2.0 基于 PPT 写讲稿）', () => {
  assertEqual(STAGE_REQUIREMENTS.lecture, [
    { type: 'design_doc', stage: 'design' },
    { type: 'ppt_outline', stage: 'ppt' },
  ], 'lecture 依赖');
});

test('video 依赖 lecture_final（v4.2.0 改为依赖讲稿）', () => {
  assertEqual(STAGE_REQUIREMENTS.video, [{ type: 'lecture_final', stage: 'lecture' }], 'video 依赖');
});
...
test('schedule + design 已确认 → 解锁 ppt（v4.2.0 改 ppt 在 lecture 之前）', () => {
  const artifacts = [
  ...
  assertEqual(computeUnlockedStages(artifacts), ['schedule', 'design', 'ppt'], 'design 确认后');
});

test('schedule + design + ppt 已确认 → 解锁 lecture', () => {
  const artifacts = [
    { type: 'schedule_table', stage: 'schedule', confirmed: true, status: 'confirmed' },
    { type: 'design_doc', stage: 'design', confirmed: true, status: 'confirmed' },
    { type: 'ppt_outline', stage: 'ppt', confirmed: true, status: 'confirmed' }
  ];
  assertEqual(computeUnlockedStages(artifacts), ['schedule', 'design', 'ppt', 'lecture'], 'ppt 确认后');
});
...
test('lecture confirmed 但 design / ppt 缺失 → lecture 不解锁（链路断点）', () => {
  const artifacts = [
    { type: 'schedule_table', stage: 'schedule', confirmed: true, status: 'confirmed' },
    // 缺 design_doc + ppt_outline
    { type: 'lecture_final', stage: 'lecture', confirmed: true, status: 'confirmed' }
  ];
  // v4.2.0 Phase A'：lecture 依赖 design + ppt，两个都缺
  ...
  if (unlocked.includes('ppt')) throw new Error('ppt 不应解锁（design_doc 缺失）');
  if (unlocked.includes('lecture')) throw new Error('lecture 不应解锁（design + ppt 缺失）');
});

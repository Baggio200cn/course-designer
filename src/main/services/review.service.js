...
function buildReviewPrompt(script, notebookContext = {}) {
  ...
  return [
    '<task>审核下面的讲稿，输出结构化评分和问题清单。审核 9 个维度（含素材融合深度、五段式教学转化、课时连贯性、版权安全）。</task>',
    '',
    ...
    '  "shouldRevise": boolean,  // score < 7 或 referenceFusionDepth < 6 时为 true',
    ...
    '  "issues": [',
    '    {',
    '      "type": "placeholder|topic_mismatch|generic_opening|missing_steps|garbage_sentence|shallow_reference|missing_five_step|timeline_break|copyright_risk",',
...
async function reviewAndRevise({ script, notebookContext = {}, aiClient, autoRevise = true }) {
  ...
  //   - 新增（重点）：素材融合深度 < 6（且确实有素材）
  //   - 新增：五段式教学转化 < 6
  //   - 新增：课时连贯性 < 5（硬伤）
  //   - 新增：版权安全 < 5（硬伤）
  const hasReference = referenceFusionDepth >= 0;  // -1 表示无素材，跳过此维度

...
function buildPptPageImagePrompt({
  ...
  aspect,
  quality,
  courseSubject  // Phase-7.7 C8-2：课程主题简述，引导画面视觉元素选择（不烧字）
}) {
  ...
  // C8-2：courseSubject 用作"行业引导"——告诉 AI 选什么样工具/材料/场景，但不写字
  ...
  // Phase-7.7 F3（2026-04-30）：sceneMap 主题驱动改造（深度嵌入版）
  // 通用模板里的"工具与材料"会被 AI 联想到笔记本所在专业的物品（服装手工/缝纫等），跟课程主题脱节
  // 修法：每个模板嵌入 ${courseSubject}，让 AI 明白"工具/材料"指的是"该课程主题对应的"
  ...
  // 修法：① 移除具体行业 example  ② 强约束"主题第一，专业第二"
  //       ③ 列出"误用情境"反例 + 列出常见课程的正确视觉对应
...
function buildPptLockedPrompt({
  ...
  // Phase-7.7 D2 续（2026-04-30）：检测老师是否真自定义了 imagePrompt
  // 之前所有模板都是 guizang + 封面是 Hero 页 → 直接走深色纹理路径忽略 customRequirements
  // 现在：老师有实质性自定义时跳过 guizang Hero 默认路径，走通用主场景（D2 已实现）
  const _userCustomCheck = String(customRequirements || '').trim();
  const _userHasMeaningfulCustom = _userCustomCheck.length >= 30
    && !_userCustomCheck.startsWith('极简主义抽象背景纹理')
    && !_userCustomCheck.startsWith('课程封面主视觉：专业实训工作台');

  // ── guizang Hero 页分路由（与 buildPptPageImagePrompt 逻辑保持一致）───────
  // D2 续：仅当老师没实质自定义时才走深色纹理；自定义时让老师诉求生效（落到下方通用路径）
  if (safeTemplate.variant === 'guizang' && GUIZANG_HERO_TYPES.has(page.pageType)
      && !_userHasMeaningfulCustom) {
    const textureDesc = GUIZANG_HERO_TEXTURE_MAP[safeTemplate.heroBackground]
      || '深色极简纹理背景，接近全黑，微弱光影层次，沉稳克制';
    return [
      `极简主义抽象背景纹理：${textureDesc}。`,
  ...
  // Phase-7.7 F3（2026-04-30）：sceneMap 主题驱动改造
  // 之前的"通用职教模板"会让 doubao 联想到笔记本所在专业（如服装手工/缝纫），跟课程主题脱节
  // 现在：每个模板嵌入 ${courseSubject}，让 AI 明白"工具/材料/成品"是该课程主题对应的
  const _ctxSubject = courseName ? `「${String(courseName).slice(0, 50)}」` : '本课';
  const sceneMap = {
    '封面':    `课程封面主视觉：${_ctxSubject}对应的专业工作环境精美俯拍，与课程主题直接相关的工具/界面/材料的精致陈列特写，构图大气，光影层次丰富，以与主题契合的物品和环境为主体`,
    '课程导入': `课堂开场氛围：宽敞明亮的实训空间全景，与${_ctxSubject}主题相关的学习设备整齐排列，探索感与好奇心，暖光，人物如出现须为远景小比例配角`,
    '路线图':  '抽象路径进程可视化：几何节点依次串联，流动的阶段感，简洁的方向感构图，无文字标注，纯抽象图形',
    '模块页':  `章节开幕主视觉：与${_ctxSubject}主题相关的单一主体强构图，视觉张力十足，画面上下留白充足，色彩饱满，以契合主题的物品或抽象图形为主体`,
    '模块导入': `真实职业现场环境全景：${_ctxSubject}对应的真实行业工作台与专业器材，自然光线，代入感强，无人物正面特写`,
    '原理讲解': `${_ctxSubject}相关的抽象概念图解：与主题概念相关的几何体结构与空间关系，对称或放射构图，纯视觉场景，无文字标签，无人物`,
    '操作步骤': `${_ctxSubject}的实操工艺俯拍特写：与课程主题对应的专业工具与材料在操作台面上的精确摆放，仅展示手部操作细节（手腕以下），材料纹理细节，禁止出现人物面部`,
    '验收检查': `${_ctxSubject}的精致成品陈列特写：光线打亮成品细节，干净的展示台背景，以与主题对应的成品为绝对视觉主体，无人物面部`,
    '课堂练习': `${_ctxSubject}的协作实训工作坊全景：多人远景活动场景，与主题相关的工具材料散落有序，活跃课堂气氛，人物为远景背景配角，禁止正面人物头像`,
    '总结收束': `${_ctxSubject}的完成收束画面：整洁陈列的与主题对应的成品序列，成就感与收获感，柔和结束氛围，以成品为主体`,
    '内容页':  `${_ctxSubject}相关的现代化职业实训工作空间：与课程主题对应的专业工具与材料精致陈列，干净整洁的构图，以环境与器材为主体`
  };
  const scene = sceneMap[page.pageType] || sceneMap['内容页'];

  // Phase-7.7 D2（2026-04-30）：让用户改的 imagePrompt 真生效
  // 之前 bug：customRequirements 只被取最后一行 + 截断 80 字 → 用户改的 99% 内容被丢弃
  // 现在：判断是否是"用户实质性自定义"（≥30 字），是则用它替代 sceneMap 主场景
  //       但仍保留人物规则 + 零文字铁律（这些是不可破坏的硬约束）
  const userCustom = String(customRequirements || '').trim();
  const userHasMeaningfulCustom = userCustom.length >= 30
    // 排除"明显是 buildPptPageImagePrompt 默认输出"的情况——
    // 默认 prompt 含特定模板词如"极简主义抽象背景纹理"或固定 sceneMap 描述，
    // 老师真实自定义通常包含"主题/场景/色调"等具体诉求关键词
    && !userCustom.startsWith('极简主义抽象背景纹理')
    && !userCustom.startsWith('课程封面主视觉：专业实训工作台');

  // 主场景：优先用户自定义（截断到 600 字防过长），否则用 sceneMap
  const mainScene = userHasMeaningfulCustom
    ? `【场景描述（老师手动编辑，请严格遵守）】${userCustom.slice(0, 600)}`
    : `场景：${scene}。`;
...
function ensurePptImagePromptForPage(page, { courseName, template, imageAspect, imageQuality }) {
  ...
  return {
    ...
    imagePrompt: buildPptPageImagePrompt({
      ...
      courseSubject: courseName  // C8-2：课程主题作为视觉引导
    })
  };
}
